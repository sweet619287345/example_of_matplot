...
host.set_ylim(185, 310)

host.set_xlabel("Number of received packet [-]", size=14)
host.set_ylabel("Time duration for each packet [µs]", size=14)

host.set_xticks([i for i in range(n[0], n[1]+1, int((n[1]-n[0])/12))])
host.set_yticks([i for i in range(185, 311, 25)])
for i in range(0, n[1]-n[0]+1, int((n[1]-n[0])/12)):
    host.annotate(s='%.6f'%time[i+n[0]], xy=(i, time[i+n[0]]), xytext=(i, 20), rotation=45, color="mediumseagreen", size=14)
...
par1.set_xlabel("Timestamp of received packet [s]", color="mediumseagreen",size=14)
#par1.grid(color="mediumturquoise", linestyle="--", axis="x")
host.set_title("%s\n Ave. %lf µs" % (sys.argv[4], average), size=14)
